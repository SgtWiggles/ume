...
ume - A simple but powerful libvte-based terminal emulator.
...
ume is a terminal emulator based on GTK+ and VTE. It's a terminal emulator with few dependencies, so you don't need a full GNOME desktop installed to have a decent terminal emulator.
...
Use alternate configuration file. Path is relative to the ume config dir.
(Example: ~/.config/ume/FILENAME).
...
B<ume> supports keyboard bindings. They are configurable by setting the following properties in your ume config file (~/.config/ume/ume.conf).
...
Before ume used keycodes instead of strings. They're still valid.	
...
B<ume> is hosted on Launchpad. Bugs can be filed at:

https://bugs.launchpad.net/ume

=head1 AUTHORS

B<ume> was written by David Gómez <david@pleyades.net>. 
